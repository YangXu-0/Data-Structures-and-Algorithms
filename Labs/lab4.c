...
PlayerRecord* create_leaf_record(Player* p){
	...
	*/
	...
	PlayerRecord *new = (PlayerRecord *)malloc(sizeof(PlayerRecord));
	new->player = p;
	new->game_records[WINS] = 0;
	new->game_records[LOSSES] = 0;
	new->left_child = NULL;
...
PlayerRecord* add_match(
	...
	*/
	...
	if (p1_wins == 13) {
		...
		new->game_records[WINS] = p1_wins;
		new->game_records[LOSSES] = p2_wins;
	}
	else {
		winner = p2;
		loser = p1;

		new->game_records[WINS] = p2_wins;
		new->game_records[LOSSES] = p1_wins;
	}

	winner->parent = new;
	loser->parent = new;

	// left child is always loser, right child is always winner
	new->left_child = loser;
	new->right_child = winner;

	new->player = winner->player;
...
int get_player_rank(char player_id[], PlayerRecord* root){
	...
	*/
	...
	if (strcmp(player_id, root->player->id) == 0) {
		return 1;
	}
	...
	int left_search = get_player_rank(player_id, root->left_child);
	int right_search = get_player_rank(player_id, root->right_child);

	if (left_search * right_search == 1) {
		// If not left or right (each will be negative 1)
		// Assuming each id only appears once
		return -1;
	}
	else {
